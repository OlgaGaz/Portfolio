...
- Проджект-менеджер,
- Аналитик,
- Продукт-оунер - лицо принимающее решение (ЛПР).
- Тим-лид команды разработки.

## Процесс

На предварительных обсуждениях продукт-оунер и проджект-менеджер рассказали о продукте, его пользователях, конкурентах, поделились ссылками на артефакты существующей версии и требованиями к новой функциональности.
...
Аналитикой пользователей на этом проекте я не занималась, т.к. проджект-менеджер достаточно хорошо исследовал своих клиентов. Все, что мне было необходимо, мне рассказывал менеджер.
...
1. **Навигация**. 
    ...
    - Второй уровень - выбор группы. 
        ...
        Далее мы рассматривали несколько вариантов размещения кнопки для добавления новых сущностей: на панели с названием дерева и отдельно над деревьями. В конечном итоге было принято разместить кнопку над деревьями.

        _**Дисклеймер**. От контекстного меню и от действия с группами в боковом меню было решено отказаться._
...
2. **Таблицы**. 
    ...
    - **Пагинация или бесконечная прокрутка?** К решению этого вопросам был привлечен тим-лид команды разработки. В результате мы остановились на “бесконечной прокрутке”.
...
Не буду останавливаться на описании этого этапа. С ним не возникло сложностей. Все мои предложения были приняты продукт-оунером. Что касается навигации, решено было остановиться на первом варианте с размещением навигации в верхней панели.

### UI-kit и сбор реферов

В качестве визуального ориентира ЛПР обозначил посадку старой версии продукта. На этом этапе я собрала локальный UI-kit. Стоит отметить, что у меня не было задачи прорабатывать полноценную дизайн-систему. 
...
6. **Действия с элементами списка**.
    ...
    - Групповые действия. Я выяснила, что пользователи редко обращаются к сценариям с групповыми действиями. Кроме того, групповые действия доступны не всем ролям в системе. Поэтому я предложила вызывать кнопки действий при проставлении отметки чекбоксом.  А для ролей, для которых эти действия недоступны, чекбоксы скрыть из интерфейса.
        ...
        ![Таблицы](./images/01-case-table-device-actions-more.png "Таблица пользователей")

        ![Таблицы](./images/01-case-table-device-actions.png "Таблица пользователей")
...
К сожалению, у меня нет информации о положении дел на текущий момент.
